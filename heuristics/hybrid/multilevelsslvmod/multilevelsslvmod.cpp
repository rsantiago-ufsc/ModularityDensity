#include "multilevelsslvmod.h"


MultiLevelSSLVMod::MultiLevelSSLVMod(LargeGraph * graph, unsigned typeLouvain)
{
    this->graph = graph;
...
void MultiLevelSSLVMod::execute(){
    ...
    LouvainSSELV lvlg(this->graph, sslg.gcss, MLSSLV_MOD);
...
void MultiLevelSSLVMod::reorganization(GraphCoarsenerSingleStepLG *&gcss, vector <unsigned> & commByVertice){
...
void MultiLevelSSLVMod::executeFromBest(){
    ...
    LouvainSSELV lvlg(this->graph, gcBest, MLSSLV_MOD);
...
void MultiLevelSSLVMod::executeAlwaysSingleton(){
    this->bestDensity = MINUS_INFINITY;
...
cout.flush();*/
        ...
        //intensification
        LouvainSSELV lvlg(this->graph, gcSingle, MLSSLV_MOD);
        lvlg.execute(commByVertices);
        if (this->bestDensity < lvlg.bestDensity){
            this->bestDensity = lvlg.bestDensity;
        }
