...
public:
    ...
    float lambda = 0.5; //quantitative parameter

    unsigned typePrioritizer;

    LMBDLouvain(LargeGraph * graph, unsigned type = LV_PRI_MOD_DELTA);
    LMBDLouvain(Solution * sol, LargeGraph * graph, unsigned type = LV_PRI_MOD_DELTA);
