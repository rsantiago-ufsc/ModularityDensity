#include "lmbdlouvain.h"

#include "../../../utils/modularitylg.h"



LMBDLouvain::LMBDLouvain(LargeGraph * graph, unsigned type)
{

    this->graph = graph;
    this->typePrioritizer = type;

    this->gl = new LMBDGraphLouvain(graph);
...
LMBDLouvain::LMBDLouvain( Solution *sol, LargeGraph * graph, unsigned type)
{
...
long double LMBDLouvain::calculateDensity(){
    ...
    if (  this->typePrioritizer == LV_PRI_DNS_DELTA
        ...
        for (unsigned i=0 ; i<this->gl->size ; i++) {
            if (this->communities[i].nnodes>0){
                ...
                */
                dens+= (4.0*this->lambda*this->communities[i].internalEdges - (2-2*this->lambda)*(this->communities[i].degree-this->communities[i].internalEdges*2.0))
                        /this->communities[i].nnodes;
...
long double LMBDLouvain::calculateModularity(){
    long double dens  = 0.0;
    long double m = this->graph->numberOfEdges;
    long double m2 = this->graph->numberOfEdges*2.0;
    //##we can avoid O(n) computation here by updating the vector communities at each pass
...
string LMBDLouvain::currentToString(){
    vector <string> comms;
...
void LMBDLouvain::neighsPrepare(unsigned node){
...
void LMBDLouvain::removeFromCommunity(unsigned node, unsigned comm, long double dnodecomm) {
...
void LMBDLouvain::insertInCommunity(unsigned node, unsigned comm, long double dnodecomm) {
    this->communities[comm].degree += this->gl->nodes[node].totalDegree;
    this->communities[comm].nnodes += this->gl->nodes[node].nodes->size();
...
long double LMBDLouvain::calculateDensityGain(unsigned node, unsigned comm, long double dnodecomm, long double wdegree) {
...
long double LMBDLouvain::calculateDensityGain2(unsigned node, unsigned comm, long double dnodecomm, long double wdegree) {
    if(this->typePrioritizer == LV_PRI_SAN_DELTA){
        long double n = this->communities[comm].nnodes;
...
void LMBDLouvain::updateLouvainGraphAndCommunities(){
...
Solution * LMBDLouvain::bestToSolution(){
