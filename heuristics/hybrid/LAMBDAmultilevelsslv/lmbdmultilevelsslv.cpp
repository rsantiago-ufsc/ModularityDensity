#include "lmbdmultilevelsslv.h"


LMBDMultiLevelSSLV::LMBDMultiLevelSSLV(LargeGraph * graph, float lambda, unsigned typeLouvain)
{
    this->lambda = lambda;
    this->graph = graph;
...
void LMBDMultiLevelSSLV::execute(){
    ...
    LMBDLouvainSSELV lvlg(this->graph, sslg.gcss, lambda);
...
void LMBDMultiLevelSSLV::reorganization(LMBDGraphCoarsenerSingleStepLG *&gcss, vector <unsigned> & commByVertice){
...
void LMBDMultiLevelSSLV::executeFromBest(){
    ...
    LMBDLouvainSSELV lvlg(this->graph, gcBest, lambda);
...
void LMBDMultiLevelSSLV::executeAlwaysSingleton(){
    this->bestDensity = MINUS_INFINITY;
...
cout.flush();*/
        ...
        //intensification
        LMBDLouvainSSELV lvlg(this->graph, gcSingle, lambda);
        lvlg.execute(commByVertices);
        if (this->bestDensity < lvlg.bestDensity){
            this->bestDensity = lvlg.bestDensity;
        }
