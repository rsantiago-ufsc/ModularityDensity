...
LMBDLouvainSSELV::LMBDLouvainSSELV(LargeGraph * graph, LMBDGraphCoarsenerSingleStepLG * gcss, float lambda, unsigned type)
{
...
void LMBDLouvainSSELV::update(LMBDGraphCoarsenerSingleStepLG * gcss){
...
long double LMBDLouvainSSELV::calculateDensity(){
    ...
    for (unsigned i=0 ; i<this->gl->size ; i++) {
        if (this->communities[i].nnodes>0){
            ...
            dens +=(4.0*lambda*this->communities[i].internalEdges
                    -(2.0-2.0*lambda)*(this->communities[i].degree
                                       - 2.0*this->communities[i].internalEdges))
                    /this->communities[i].nnodes;
...
long double LMBDLouvainSSELV::calculateModularity(){
    long double mod  = 0.0;
...
bool LMBDLouvainSSELV::oneLevel(){
...
string LMBDLouvainSSELV::currentToString(){
    vector <string> comms;
...
void LMBDLouvainSSELV::neighsPrepare(unsigned node){
...
void LMBDLouvainSSELV::removeFromCommunity(unsigned node, unsigned comm, long double dnodecomm) {
...
void LMBDLouvainSSELV::insertInCommunity(unsigned node, unsigned comm, long double dnodecomm) {
    this->communities[comm].degree += this->gl->nodes[node].totalDegree;
    this->communities[comm].nnodes += this->gl->nodes[node].nodes.size();
...
long double LMBDLouvainSSELV::calculateDensityGain(unsigned node, unsigned comm, long double dnodecomm, long double wdegree) {
...
*/
    ...
    return  (
               +4.0 * this->communities[comm].nnodes * dnodecomm
               -2.0 * this->communities[comm].nnodes * wdegree
               +2.0 * this->communities[comm].nnodes * wdegree * lambda
               +2.0 * this->gl->nodes[node].nodes.size() * this->communities[comm].degree
               -2.0 * this->gl->nodes[node].nodes.size() * this->communities[comm].degree * lambda
               -4.0 * this->gl->nodes[node].nodes.size() * this->communities[comm].internalEdges
                    ) /
        ( this->communities[comm].nnodes * (this->communities[comm].nnodes + this->gl->nodes[node].nodes.size()) );
}


long double LMBDLouvainSSELV::calculateModularityGain(unsigned node, unsigned comm, long double dnodecomm, long double wdegree) {
    long double m2   = this->graph->numberOfEdges*2.0;
    return (dnodecomm - this->communities[comm].degree * wdegree / m2);
}

void LMBDLouvainSSELV::execute(vector<unsigned> & commByVertice){
...
void LMBDLouvainSSELV::updateLouvainGraphAndCommunities(vector<unsigned> &commByVertice){
