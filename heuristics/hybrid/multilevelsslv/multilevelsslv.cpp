#include "multilevelsslv.h"


MultiLevelSSLV::MultiLevelSSLV(LargeGraph * graph, unsigned typeLouvain)
{
    this->graph = graph;
...
void MultiLevelSSLV::execute(){
    chrono::system_clock::time_point before;
    before = chrono::system_clock::now();

    long long int tInicio, tUnmerge=0, tReorg=0, tIntens=0;
    chrono::system_clock::time_point before2 = chrono::system_clock::now();
    ...
    LouvainSSELV lvlg(this->graph, sslg.gcss);
   ...
   tInicio = chrono::duration_cast<chrono::milliseconds>(chrono::system_clock::now()-before2).count();

   for (int l=sslg.levelDetails.size()-1; l>=0; l--){

        this->it++;
        //uncoarsening
before2 = chrono::system_clock::now();
        sslg.gcss->unmerge(sslg.levelDetails[l], lvlg.commByNode, commByVertices,this->it);
tUnmerge += chrono::duration_cast<chrono::milliseconds>(chrono::system_clock::now()-before2).count();

        //reorganizing the blank communities
before2 = chrono::system_clock::now();
        this->reorganization(sslg.gcss, commByVertices);
tReorg += chrono::duration_cast<chrono::milliseconds>(chrono::system_clock::now()-before2).count();

        //intensification
before2 = chrono::system_clock::now();
        lvlg.update(sslg.gcss);
        lvlg.execute(commByVertices);
tIntens += chrono::duration_cast<chrono::milliseconds>(chrono::system_clock::now()-before2).count();
...
/*   cout<<"\nStarting time     (seg): "<<tInicio/1000.0;
   cout<<"\nUnmerging time    (seg): "<<tUnmerge/1000.0;
   cout<<"\nReorganizing time (seg): "<<tReorg/1000.0;
   cout<<"\nIntensifying time (seg): "<<tIntens/1000.0;
   cout<<"\n\t-->OneLevel (seg): "<<lvlg.timeOneLevel/1000.0;
   cout<<"\n\t-->\t-->initPrep.   (seg): "<<lvlg.timeOL1/1000.0;
   cout<<"\n\t-->\t-->neighsPrep. (seg): "<<lvlg.timeOL2/1000.0;
   cout<<"\n\t-->\t-->evalNeighs. (seg): "<<lvlg.timeOL3/1000.0;
   cout<<"\n\t-->\t-->calcDens.   (seg): "<<lvlg.timeOL4/1000.0;
   cout<<"\n\t-->\t-->copyBest.   (seg): "<<lvlg.timeOL5/1000.0;
   cout<<"\n\t-->\t--> 6          (seg): "<<lvlg.timeOL6/1000.0;
   cout<<"\n\t-->\t--> 7          (seg): "<<lvlg.timeOL7/1000.0;
   cout<<"\n\t-->Update   (seg): "<<lvlg.timeUpdate/1000.0;
*/
}


void MultiLevelSSLV::reorganization(GraphCoarsenerSingleStepLG *&gcss, vector <unsigned> & commByVertice){
...
void MultiLevelSSLV::executeFromBest(){
...
void MultiLevelSSLV::executeAlwaysSingleton(){
    this->bestDensity = MINUS_INFINITY;
    ...
    //identifying where is each node
    GraphCoarsenerSingleStepLG * gcSingleAux = new GraphCoarsenerSingleStepLG(this->graph, NULL, TYPE_PRI_DENSITY, false);
    GraphCoarsenerSingleStepLG * gcSingle = new GraphCoarsenerSingleStepLG(this->graph, NULL, TYPE_PRI_DENSITY, false);
    vector <unsigned> commByVertices (this->graph->numberOfNodes);
    unordered_set<unsigned>::iterator it;
    for (unsigned i=0;i<this->graph->numberOfNodes; i++){
        commByVertices[i] = i;
...
cout.flush();*/
        ...
        //intensification
        LouvainSSELV lvlg(this->graph, gcSingle);
        lvlg.execute(commByVertices);
        if (this->bestDensity < lvlg.bestDensity){
            this->bestDensity = lvlg.bestDensity;
        }
